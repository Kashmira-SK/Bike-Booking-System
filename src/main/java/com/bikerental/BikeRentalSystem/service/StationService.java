package com.bikerental.BikeRentalSystem.service;

import com.bikerental.BikeRentalSystem.model.Station;
import com.bikerental.BikeRentalSystem.util.FileHelper;
import java.io.IOException;
import java.util.List;

public class StationService {
    private static final String FILE_PATH = "data/stations.txt";

    public void createStation(Station station) throws IOException {
        FileHelper.appendLine(FILE_PATH, station.toFileString());
    }

    public List<String> readAll() throws IOException {
        return FileHelper.readLines(FILE_PATH);
    }
}
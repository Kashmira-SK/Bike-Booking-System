...
public class RentalService {
    ...
    private double calculateCost(Rental rental, double pricePerHour) {
        if (rental.getEndTime() == null || rental.getEndTime().isEmpty())
            return 0;
    ...
    public boolean rentBike(String userId, String bikeId, String startStation, String endStation, String type) {
        ...
        if (requestedBikeId == null || !requestedBikeId.equals(bikeId))
            return false;
        ...
        if (AppConstants.RENTAL_HOURLY.equals(type)) {
            ...
            rental = new DailyRental(id, userId, bikeId, startStation, endStation, LocalDateTime.now().format(FMT), "", 0, AppConstants.RENTAL_ACTIVE);
        }

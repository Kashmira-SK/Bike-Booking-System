<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>My Profile — BikeRental</title>

    <%-- Bootstrap 5 + Bootstrap Icons --%>
    <link rel="stylesheet"
          href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css">
    <link rel="stylesheet"
          href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">

    <style>
        body { background: #f4f6f9; }

        /* Profile avatar circle */
        .avatar-ring {
            width: 80px; height: 80px;
            border-radius: 50%;
            display: flex; align-items: center; justify-content: center;
            font-size: 2.2rem;
            background: linear-gradient(135deg, #ffc107 0%, #fd7e14 100%);
            color: #fff;
            flex-shrink: 0;
        }

        /* Role badge sizes */
        .role-badge-lg {
            font-size: 0.75rem; letter-spacing: .04em;
            padding: .35em .7em; border-radius: 999px;
        }

        /* Seller CTA card highlight */
        .seller-cta {
            border: 2px dashed #ffc107;
            background: rgba(255,193,7,.06);
        }
        .seller-active {
            border: 2px solid #198754;
            background: rgba(25,135,84,.06);
        }

        /* Quick-link card hover */
        .quick-link-card {
            transition: transform .15s, box-shadow .15s;
            cursor: pointer;
            text-decoration: none;
            color: inherit;
        }
        .quick-link-card:hover {
            transform: translateY(-3px);
            box-shadow: 0 6px 20px rgba(0,0,0,.12);
            color: inherit;
        }
    </style>
</head>
<body>

<%-- ── Navbar include ─────────────────────────────────────────────────────── --%>
<%@ include file="navbar.jsp" %>

<%-- ── Page aliases ───────────────────────────────────────────────────────── --%>
<c:set var="user" value="${sessionScope.loggedInUser}" />
<c:set var="role" value="${sessionScope.loggedInUser.role}" />
<c:set var="ctx"  value="${pageContext.request.contextPath}" />

<div class="container py-5">

    <%-- ════════════════════════════════════════════════════════════════════
         SECTION 1 — Profile header card
         ════════════════════════════════════════════════════════════════ --%>
    <div class="card shadow-sm mb-4">
        <div class="card-body p-4">
            <div class="d-flex align-items-center gap-4 flex-wrap">

                <%-- Avatar --%>
                <div class="avatar-ring">
                    <i class="bi bi-person-fill"></i>
                </div>

                <%-- Name + role --%>
                <div>
                    <h2 class="mb-1 fw-bold"><c:out value="${user.name}" /></h2>
                    <span class="role-badge-lg badge
                        ${role == 'ADMIN'  ? 'bg-warning text-dark' :
                          role == 'SELLER' ? 'bg-success'           : 'bg-secondary'}">
                        <c:out value="${role}" />
                    </span>
                </div>

            </div>
        </div>
    </div>

    <div class="row g-4">

        <%-- ════════════════════════════════════════════════════════════════
             SECTION 2 — User info table
             ════════════════════════════════════════════════════════════ --%>
        <div class="col-lg-7">
            <div class="card shadow-sm h-100">
                <div class="card-header bg-white fw-semibold border-bottom py-3">
                    <i class="bi bi-person-vcard me-2 text-primary"></i>Account Information
                </div>
                <div class="card-body p-0">
                    <table class="table table-borderless mb-0">
                        <tbody>
                            <tr class="border-bottom">
                                <th class="ps-4 py-3 text-muted fw-normal" style="width:38%">
                                    <i class="bi bi-person me-2"></i>Full Name
                                </th>
                                <td class="py-3 fw-semibold">
                                    <c:out value="${user.name}" />
                                </td>
                            </tr>
                            <tr class="border-bottom">
                                <th class="ps-4 py-3 text-muted fw-normal">
                                    <i class="bi bi-envelope me-2"></i>Email
                                </th>
                                <td class="py-3">
                                    <c:out value="${user.email}" />
                                </td>
                            </tr>
                            <tr class="border-bottom">
                                <th class="ps-4 py-3 text-muted fw-normal">
                                    <i class="bi bi-telephone me-2"></i>Phone
                                </th>
                                <td class="py-3">
                                    <c:choose>
                                        <c:when test="${not empty user.phone}">
                                            <c:out value="${user.phone}" />
                                        </c:when>
                                        <c:otherwise>
                                            <span class="text-muted fst-italic">Not provided</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                            <tr class="border-bottom">
                                <th class="ps-4 py-3 text-muted fw-normal">
                                    <i class="bi bi-shield-check me-2"></i>Role
                                </th>
                                <td class="py-3">
                                    <span class="badge role-badge-lg
                                        ${role == 'ADMIN'  ? 'bg-warning text-dark' :
                                          role == 'SELLER' ? 'bg-success'           : 'bg-secondary'}">
                                        <c:out value="${role}" />
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <th class="ps-4 py-3 text-muted fw-normal">
                                    <i class="bi bi-activity me-2"></i>Status
                                </th>
                                <td class="py-3">
                                    <c:choose>
                                        <c:when test="${user.status == 'ACTIVE' or empty user.status}">
                                            <span class="badge bg-success-subtle text-success border border-success-subtle px-2">
                                                <i class="bi bi-circle-fill me-1" style="font-size:.5rem;"></i>Active
                                            </span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2">
                                                <i class="bi bi-circle-fill me-1" style="font-size:.5rem;"></i>
                                                <c:out value="${user.status}" />
                                            </span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>

        <%-- ════════════════════════════════════════════════════════════════
             SECTION 3 — Role-specific card (right column)
             ════════════════════════════════════════════════════════════ --%>
        <div class="col-lg-5 d-flex flex-column gap-4">

            <%-- ── REGULAR: "Become a Seller" CTA ───────────────────────── --%>
            <c:if test="${role == 'REGULAR'}">
                <div class="card seller-cta shadow-sm">
                    <div class="card-body p-4 text-center">
                        <div class="mb-3">
                            <i class="bi bi-shop fs-1 text-warning"></i>
                        </div>
                        <h5 class="fw-bold mb-2">Want to list your bikes?</h5>
                        <p class="text-muted mb-4" style="font-size:.92rem;">
                            Upgrade to a Seller account and start earning by sharing
                            your bikes with the community.
                        </p>
                        <a href="${ctx}/seller/register"
                           class="btn btn-warning fw-semibold px-4">
                            <i class="bi bi-arrow-up-circle me-2"></i>Become a Seller
                        </a>
                    </div>
                </div>
            </c:if>

            <%-- ── SELLER: Seller status card ────────────────────────────── --%>
            <c:if test="${role == 'SELLER'}">
                <div class="card seller-active shadow-sm">
                    <div class="card-body p-4 text-center">
                        <div class="mb-3">
                            <i class="bi bi-patch-check-fill fs-1 text-success"></i>
                        </div>
                        <h5 class="fw-bold mb-1">You're a verified Seller</h5>
                        <p class="text-muted mb-4" style="font-size:.92rem;">
                            Your seller account is active. Manage your listed bikes
                            and track your earnings below.
                        </p>
                        <a href="${ctx}/bikes/my"
                           class="btn btn-success fw-semibold px-4">
                            <i class="bi bi-list-check me-2"></i>Manage My Bikes
                        </a>
                    </div>
                </div>
            </c:if>

            <%-- ── ADMIN: Admin status card ───────────────────────────────── --%>
            <c:if test="${role == 'ADMIN'}">
                <div class="card border-warning shadow-sm"
                     style="background:rgba(255,193,7,.06);">
                    <div class="card-body p-4 text-center">
                        <div class="mb-3">
                            <i class="bi bi-shield-lock-fill fs-1 text-warning"></i>
                        </div>
                        <h5 class="fw-bold mb-1">Administrator Account</h5>
                        <p class="text-muted mb-4" style="font-size:.92rem;">
                            You have full administrative access to the platform,
                            including user management and station control.
                        </p>
                        <a href="${ctx}/admin/users"
                           class="btn btn-warning text-dark fw-semibold px-4">
                            <i class="bi bi-people me-2"></i>Admin Panel
                        </a>
                    </div>
                </div>
            </c:if>

        </div><%-- /col-lg-5 --%>
    </div><%-- /row --%>

    <%-- ════════════════════════════════════════════════════════════════════
         SECTION 4 — Quick links
         ════════════════════════════════════════════════════════════════ --%>
    <div class="mt-4">
        <h6 class="text-muted text-uppercase fw-semibold mb-3"
            style="letter-spacing:.06em;font-size:.78rem;">
            Quick Access
        </h6>
        <div class="row g-3">

            <%-- My Rentals --%>
            <div class="col-sm-6 col-md-4">
                <a href="${ctx}/rentals/my" class="card quick-link-card shadow-sm text-decoration-none">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        <div class="rounded-circle bg-primary-subtle text-primary d-flex align-items-center justify-content-center"
                             style="width:42px;height:42px;flex-shrink:0;">
                            <i class="bi bi-clock-history fs-5"></i>
                        </div>
                        <div>
                            <div class="fw-semibold">My Rentals</div>
                            <div class="text-muted" style="font-size:.82rem;">View rental history</div>
                        </div>
                    </div>
                </a>
            </div>

            <%-- My Payments --%>
            <div class="col-sm-6 col-md-4">
                <a href="${ctx}/payments/my" class="card quick-link-card shadow-sm text-decoration-none">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        <div class="rounded-circle bg-success-subtle text-success d-flex align-items-center justify-content-center"
                             style="width:42px;height:42px;flex-shrink:0;">
                            <i class="bi bi-credit-card fs-5"></i>
                        </div>
                        <div>
                            <div class="fw-semibold">My Payments</div>
                            <div class="text-muted" style="font-size:.82rem;">Billing & invoices</div>
                        </div>
                    </div>
                </a>
            </div>

            <%-- My Reviews --%>
            <div class="col-sm-6 col-md-4">
                <a href="${ctx}/reviews/my" class="card quick-link-card shadow-sm text-decoration-none">
                    <div class="card-body d-flex align-items-center gap-3 p-3">
                        <div class="rounded-circle bg-warning-subtle text-warning d-flex align-items-center justify-content-center"
                             style="width:42px;height:42px;flex-shrink:0;">
                            <i class="bi bi-star fs-5"></i>
                        </div>
                        <div>
                            <div class="fw-semibold">My Reviews</div>
                            <div class="text-muted" style="font-size:.82rem;">Ratings you've left</div>
                        </div>
                    </div>
                </a>
            </div>

            <%-- My Bikes — only for SELLER and ADMIN --%>
            <c:if test="${role == 'SELLER' or role == 'ADMIN'}">
                <div class="col-sm-6 col-md-4">
                    <a href="${ctx}/bikes/my" class="card quick-link-card shadow-sm text-decoration-none">
                        <div class="card-body d-flex align-items-center gap-3 p-3">
                            <div class="rounded-circle bg-info-subtle text-info d-flex align-items-center justify-content-center"
                                 style="width:42px;height:42px;flex-shrink:0;">
                                <i class="bi bi-bicycle fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold">My Bikes</div>
                                <div class="text-muted" style="font-size:.82rem;">Manage your listings</div>
                            </div>
                        </div>
                    </a>
                </div>
            </c:if>

            <%-- Admin Panel — only for ADMIN --%>
            <c:if test="${role == 'ADMIN'}">
                <div class="col-sm-6 col-md-4">
                    <a href="${ctx}/admin/users" class="card quick-link-card shadow-sm text-decoration-none">
                        <div class="card-body d-flex align-items-center gap-3 p-3">
                            <div class="rounded-circle bg-danger-subtle text-danger d-flex align-items-center justify-content-center"
                                 style="width:42px;height:42px;flex-shrink:0;">
                                <i class="bi bi-shield-lock fs-5"></i>
                            </div>
                            <div>
                                <div class="fw-semibold">Admin Panel</div>
                                <div class="text-muted" style="font-size:.82rem;">Users & system</div>
                            </div>
                        </div>
                    </a>
                </div>
            </c:if>

        </div><%-- /quick-link row --%>
    </div>

</div><%-- /container --%>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>